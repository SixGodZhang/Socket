...
int main()
{
	...
	*/
	...
	char bufferFile[BUF_SIZE] = { 0 };//缓冲区
	int nCount = 0;// 记录文件是否读取到结尾
	...
	while (1)
	{
		...
		sprintf(filename, "D:\\serverDir\\%s", buffer);
		cout << "文件路径: " << filename << endl;
		FILE *fp = fopen(filename, "rb"); // 以二进制的方式打开文件
		if (fp == NULL)
		{
			cout << "Can't find file " << filename << endl;
			char returnMsg[BUF_SIZE] = { 0 };
			sprintf(returnMsg, "Can't open file 【%s】", filename);
			send(clntSock, returnMsg, strlen(returnMsg), 0);//返回找文件失败问题
		...
		//循环发送数据, 直到文件结尾
		nCount = 0;
		while ((nCount = fread(bufferFile, 1, BUF_SIZE, fp)) > 0)
		{
			send(clntSock, bufferFile, nCount, 0);
		}
